...
DEFINE_string(output, "", "Output file for plotting optimized trajectory.");
DEFINE_string(compare, "", "File containing optimal solution to compare against.");
DEFINE_string(compare_output, "", "Output file for plotting comparison trajectory.");
...
struct ProblemData
{
  ...
  /**
   * Plots the first dof of the trajectory to a file
   */
  void plot_to_file(const std::string& filename)
  {
    ...
    double t = 0.0;
    //double total_duration = dt_variables.sum();
    while (true)
    {
      // ensure that we're still in the current segment
      while (t >= segment_end_time && segment_id < num_segments-1)
      {
        ++segment_id;
        segment_start_time = segment_end_time;
        segment_end_time += dt_variables[segment_id];
      }
      ...
      const int dof = 0;
      const double x0   = variables[dof](0, segment_id);
      const double xd0  = variables[dof](1, segment_id);
      const double xdd0 = variables[dof](2, segment_id);
      const double x1   = variables[dof](0, segment_id+1);
      const double xd1  = variables[dof](1, segment_id+1);
      const double xdd1 = variables[dof](2, segment_id+1);

      compute_spline_derivatives(x0, xd0, xdd0, x1, xd1, xdd1,
                                 t - segment_start_time,
                                 double(dt_variables(segment_id)),
                                 sample);
...
int main(int argc, char** argv)
{
  ...
  usage += "  ";
  usage += argv[0];
  usage += "  --input=input.txt [ --output=trajectory.txt ]\n";
  usage += "  [ --compare=compare.txt ] [ --compare-output=compare_traj.txt ]";
  ...
  {
    data.plot_to_file(FLAGS_output);
  }

  if (!FLAGS_compare.empty() && !FLAGS_compare_output.empty())
  {
    ProblemData compare;
    compare.read_from_file(FLAGS_compare);
    compare.plot_to_file(FLAGS_compare_output);
  }
