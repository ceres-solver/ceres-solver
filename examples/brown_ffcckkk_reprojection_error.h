...
// translation, and the camera intrinsics is parametrized using 7 parameters: 2
// for focal length, 2 for principal point and 3 for radial distortion.
struct BrownffcckkkReprojectionError {
  BrownffcckkkReprojectionError(const Eigen::Vector2d& observed)
      : observed(observed) {}
  ...
  static ceres::CostFunction* Create(const Eigen::Vector2d& observed) {
    return (new ceres::
                AutoDiffCostFunction<BrownffcckkkReprojectionError, 2, 7, 6, 3>(
                    new BrownffcckkkReprojectionError(observed)));
  }

  Eigen::Vector2d observed;
};

struct BrownffcckkkReprojectionErrorWithQuaternions {
  // (u, v): the position of the observation with respect to the image
  // center point.
  BrownffcckkkReprojectionErrorWithQuaternions(const Eigen::Vector2d& observed)
      : observed(observed) {}
  ...
  static ceres::CostFunction* Create(const Eigen::Vector2d& observed) {
    return (new ceres::AutoDiffCostFunction<
            BrownffcckkkReprojectionErrorWithQuaternions,
            2,
            7,
            7,
            3>(new BrownffcckkkReprojectionErrorWithQuaternions(observed)));
  }
