...
class ExponentialLoss : public ceres::PositiveFunction<2> {
 public:
  virtual ~ExponentialLoss() {}
  virtual bool EvaluateCostAndGradient(const double* parameters,
    ...
    for (int i = 0; i < kNumObservations; ++i) {
      const double x = data[2 * i];
      const double y = data[2 * i + 1];
      const double f = exp(m * x + c);
      const double r = y - f;
...
int main(int argc, char** argv) {
  ...
  ceres::PositiveFunctionSolver(options,
                                loss,
                                parameters,
                                &summary);
