...
void TestLinearizationState(const MarginalizationOptions& options) {
  ...
  problem.AddResidualBlock(
      ...
      nullptr,
      &states[0]);
  const std::set<double*> parameter_blocks_to_marginalize = {&states[0]};
  ...
  EXPECT_TRUE(MarginalizeOutVariables(options,
  ...
  const auto* marginalization_prior =
      dynamic_cast<const MarginalizationPriorCostFunction*>(
          problem.GetCostFunctionForResidualBlock(marginalization_prior_id));
 ...
 public:
  ...
  TestGraphState() {
    ...
    for (int k = 0; k < kNumBlocks; ++k) {
      ...
      Manifold* manifold =
          k <= 1 ? static_cast<Manifold*>(new SubsetManifold(block_size, {1}))
                 : static_cast<Manifold*>(new EuclideanManifold(block_size));
...
TEST(Marginalization, MarginalizationFailureInfeasible) {
  double x = 4.0;
  double y = 2.0;
  Problem problem;
