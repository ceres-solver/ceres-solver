...
TEST(Marginalization, Simple) {
  ...
  // Marginalize out x
  MarginalizeOutVariables(MarginalizationOptions(),  // Default options
...
TEST(Marginalization, LinearizationState) {
  ...
  options.assume_marginal_information_is_full_rank = false;
  ASSERT_NO_FATAL_FAILURE(TestLinearizationState(options));
  options.assume_marginal_information_is_full_rank = true;
  ASSERT_NO_FATAL_FAILURE(TestLinearizationState(options));
}
class TestGraphState {
  ...
  //  +-----x4
  //
  // All cost functions are random linear cost functions. The Euclidean and
  // subset manifolds are used.
 public:
  ...
  void MarginalizeOutVariableSubset(
    ...
    const bool success = MarginalizeOutVariables(
        options_,
...
TEST(Marginalization, Success) {
  ...
  for (int trial = 0; trial < kNumTrials; ++trial) {
    for (int assume_marginalized_block_is_full_rank;
         assume_marginalized_block_is_full_rank < 2;
         ++assume_marginalized_block_is_full_rank) {
      for (int assume_marginal_information_is_full_rank;
           assume_marginal_information_is_full_rank < 2;
           ++assume_marginal_information_is_full_rank) {
        for (int use_different_states_for_linearization;
             use_different_states_for_linearization < 2;
             ++use_different_states_for_linearization) {
          for (unsigned int m = 1; m < (1u << TestGraphState::kNumBlocks) - 1;
               m++) {
            MarginalizationOptions options;
            options.assume_marginal_information_is_full_rank =
                static_cast<bool>(assume_marginal_information_is_full_rank);
            ...
            ASSERT_NO_FATAL_FAILURE(TestMarginalization(
                options,
                !static_cast<bool>(use_different_states_for_linearization),
                m));
          }
        }
      }
...
TEST(Marginalization, SuccessMarginalizedBlockNotFullRank) {
  ...
  EXPECT_TRUE(MarginalizeOutVariables(
      options, {&x[0], &x[1]}, &problem, &marginalization_prior_ids, nullptr));
  const auto* marginalization_prior =
      static_cast<const MarginalizationPriorCostFunction*>(
          problem.GetCostFunctionForResidualBlock(
              marginalization_prior_ids.front()));
...
TEST(Marginalization, FailureNoMarkovBlanket) {
  ...
  problem.AddResidualBlock(
      ...
      x);
  MarginalizationOptions options;
  EXPECT_FALSE(MarginalizeOutVariables(options,
                                       {x},  // marginalize x
                                       &problem,
                                       nullptr));
}
...
TEST(Marginalization, FailureMarkovBlanketIsInfeasible) {
  double x = 4.0;
  double y = 2.0;
  Problem problem;
  problem.AddParameterBlock(&x, 1);
...
TEST(Marginalization, FailureLocalParameterizationUsed) {
  // If linearization states are provided, they must include all variables.
  double x = 4.0;
  double y = 2.0;
  Problem problem;
  problem.AddParameterBlock(&x, 1, new IdentityParameterization(1));
  problem.AddParameterBlock(&y, 1);
  problem.AddResidualBlock(new NonlinearBinaryCostFunction(), nullptr, &x, &y);
