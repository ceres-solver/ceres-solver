...
TEST(Marginalization, SlidingWindowOptimization) {
  ...
  for (unsigned int t = 0; t < T; t++) {
    ...
    if (t >= window_size) {
      ...
      Marginalization marginalizer;
      EXPECT_TRUE(marginalizer.MarginalizeOutVariables(
          parameter_blocks_to_marginalize, pProblem.get()));
    }
...
class TestGraphState {
 ...
 public:
  ...
  void MarginalizeOutVariables(
    ...
    Marginalization marginalizer;
    EXPECT_TRUE(marginalizer.MarginalizeOutVariables(
        parameter_blocks_to_marginalize, &problem_));
  }
...
TEST(Marginalization, MarginalizationFailure) {
  ...
  EXPECT_FALSE(marginalizer.MarginalizeOutVariables(to_marginalize, &problem));
