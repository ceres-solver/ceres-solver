...
static bool ComputeMarginalFactorData(
  ...
  EvaluateProblem(parameter_blocks_to_marginalize.size(),
                  parameter_block_linearization_states, problem.get(), &jtj,
  ...
  // TODO: the dimension of the residual can be reduced with an
...
static bool Compute(
    const set<double*>& parameter_blocks_to_marginalize,
    const map<double*, const double*>* parameter_block_linearization_states,
    Problem* problem, vector<double*>* markov_blanket_parameter_blocks,
...
static bool MarginalizeOutVariables(
  ...
  LinearCostFunction* marginal_factor_cost_function;
  if (!Compute(parameter_blocks_to_marginalize,
