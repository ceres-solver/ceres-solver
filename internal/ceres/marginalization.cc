...
static int SumGlobalSize(const std::vector<ParameterBlock*>& parameter_blocks) {
  int sum = 0;
...
static int SumLocalSize(const std::vector<ParameterBlock*>& parameter_blocks) {
  int sum = 0;
  for (int b = 0; b < parameter_blocks.size(); b++) {
    auto* p = parameter_blocks[b];
    sum += p->LocalSize();
...
static Matrix ComputeSqrtFactor(const Eigen::LDLT<Matrix>& ldlt) {
  ...
  // Diagonal elements can be very small negative.
...
// Compute A^T * A for a sparse matrix A.
...
static ProblemImpl* BuildProblemWithMarginalizedBlocksAndMarkovBlanket(
  ...
  std::set<ResidualBlockId> marginalized_blocks_residual_ids;
...
// Get an ordering with parameter blocks to be marginalized followed by
...
static ParameterBlockOrdering GetOrderingForMarginalizedBlocksAndMarkovBlanket(
  ...
  std::vector<double*> added_parameter_blocks;
...
static bool ComputeMarginalFactorData(
    const Problem& external_problem,
    const std::set<double*>& parameter_blocks_to_marginalize, Matrix* jacobian,
    Vector* b, std::vector<double*>* ordered_parameter_block_ptrs,
  ...
  *b = sqrt_factor.transpose() *
  ...
  for (auto* parameter_block : ordered_parameter_blocks_blanket) {
    ordered_parameter_block_ptrs->push_back(
...
void MarginalizeOutVariables(
    const std::set<double*>& parameter_blocks_to_marginalize,
    Problem* problem) {
  const MarginalFactorData marginal_factor_data(
      *problem, parameter_blocks_to_marginalize);
  for (auto it = parameter_blocks_to_marginalize.begin();
       it != parameter_blocks_to_marginalize.end(); ++it) {
    problem->RemoveParameterBlock(*it);
  }
  const auto markov_blanket_parameter_blocks =
      marginal_factor_data.GetOrderedParameterBlocks();
  problem->AddResidualBlock(
      new MarginalFactorCostFunction(marginal_factor_data), NULL,
      markov_blanket_parameter_blocks.data(),
      static_cast<int>(markov_blanket_parameter_blocks.size()));
}

MarginalFactorData::MarginalFactorData(
    const Problem& external_problem,
    const std::set<double*>& parameter_blocks_to_marginalize) {
  internal::ComputeMarginalFactorData(
      external_problem, parameter_blocks_to_marginalize, &jacobian_, &b_,
      &ordered_parameter_block_ptrs_, &parameter_block_sizes_);
}
