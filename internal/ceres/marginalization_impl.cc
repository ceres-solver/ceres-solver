...
static void SchurComplement(const Matrix& jtj, const Vector& gradient,
  ...
  const Matrix jtj_11_inv = InvertPSDMatrix<Eigen::Dynamic>(true, jtj_11);
  *jtj_marginal = jtj_22 - jtj_12.transpose() * jtj_11_inv * jtj_12;
  *gradient_marginal = gradient2 - jtj_12.transpose() * jtj_11_inv * gradient1;
}

// Compute the parameters of MarginalFactorCostFunction and return the blocks
...
MarginalizationImpl::MarginalizationImpl() {}

MarginalizationImpl::~MarginalizationImpl() {}

bool MarginalizationImpl::MarginalizeOutVariables(
    const set<double*>& parameter_blocks_to_marginalize, Problem* problem) {
  vector<double*> markov_blanket_ordered_parameter_blocks;
  MarginalFactorCostFunction* marginal_factor_cost_function;
...
bool MarginalizationImpl::Compute(
    const set<double*>& parameter_blocks_to_marginalize, Problem* problem,
  ...
  *marginal_factor_cost_function = new MarginalFactorCostFunction(
      marginal_factor_jacobian, marginal_factor_b,
      marginal_factor_parameter_block_sizes);
  return true;
}
