 ...
 public:
  MarginalizationPriorCostFunction(
      ...
      const std::vector<std::vector<double>>&
          parameter_block_reference_points,  // x0
    ...
    for (size_t i = 0; i < parameter_block_manifolds.size(); i++) {
      int tan_size = parameter_block_tan_sizes[i];
  ...
  virtual bool Evaluate(double const* const* parameters,
    ...
    for (int j = 0; j < num_parameter_blocks; ++j) {
      const int tan_size = static_cast<int>(parameter_block_tan_sizes_[j]);
