...
Wedin present a simpler algorithm with comparable comparable
convergence properties, which they refer to as Algorithm II. In this
algorithm, once the Newton/Trust Region step has been computed for the
whole problem $(a_1, a_2, b_1, b_2, c_1)$ an additional optimization
step is performed to estimate $a_1$ and $a_2$ exactly.
...
If the user leaves the choice upto Ceres, then the solver uses an
