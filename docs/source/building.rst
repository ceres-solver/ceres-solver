...
- `ProjectConfig tutorial <http://www.cmake.org/Wiki/CMake/Tutorials/How_to_create_a_ProjectConfig.cmake_file>`_
   ...
   Covers how to write a ``ProjectConfig.cmake`` file, discussed below, for
...
but ``Bar`` will not link properly, as it does not have Ceres required public
link dependencies which are stored in the imported target defintion.
