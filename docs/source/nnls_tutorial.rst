...
#. `slam/pose_graph_3d/pose_graph_3d.cc
   ...
   functionality. To represent the orientation, we will use Eigen's quaternion
   which uses the Hamiltonian convention but uses a different internal memory
   layout for the elements of the quaternion than what is commonly used.
   Specifically, Eigen stores the elements in memory as :math:`[x, y, z, w]`
   where the real part is last whereas it is typically stored first. Note, when
   creating an Eigen quaternion through the constructor the elements are
   accepted in :math:`w`, :math:`x`, :math:`y`, :math:`z` order. Since Ceres
   operates on parameter blocks which are raw double pointers this difference is
   ...
   orientation represented as a Hamiltonian quaternion. The measurement of the
   ...
   only apply updates orthongal to the 4-vector defining the quaternion. Note,
   we cannot use the :class:`QuaternionParameterization` that is included in
   Ceres since Eigen uses a different parameter ordering.
   ...
   uses the g2o format with quaternions used for the orientation
   representation. It would be relatively straightforward to implement a new
   reader for a different format such as TORO or others. ``pose_graph_3d`` will
   print the Ceres solver full summary and then output to disk the original and
   optimized poses (``poses_original.txt`` and ``poses_optimized.txt``,
