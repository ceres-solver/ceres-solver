...
#. `slam/pose_graph_3d/pose_graph_3d.cc
   ...
   which uses the Hamiltonian convention but has different element ordering as
   compared with Ceres's internal rotation representation.  Specifically they
   differ by whether the scalar component :math:`q_w` is first or last; the
   element order for Ceres's quaternion is :math:`[q_w, q_x, q_y, q_z]` where as
   Eigen's quaternion is :math:`[q_x, q_y, q_z, q_w]`.
