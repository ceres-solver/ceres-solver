...
#. `slam/pose_graph_2d/pose_graph_2d.cc
   ...
   building a map of an unknown environment while simultaneously localizing
   against this map. The main difficulty of this problem stems from not having
   any additional external aiding information such as GPS. SLAM has been
   considered one of the fundamental challenges of robotics. There are many
   resources on SLAM [#f9]_. The following explains how to formulate the
   traditional graph-based SLAM problem in 2-Dimensions with relative pose
   constraints.
   ...
   raw measurements, we want estimate the trajectory of the robot as well as
   ...
   .. figure:: slam2d.png
      ...
      Graphical representation of the Graph SLAM formulation.
   ...
   transform between the robot state at two timestamps :math:`i` and :math:`j`
   is given as: :math:`z_{ij} = [\hat{p}_{ij}^T, \hat{\psi}_{ij}]`. The residual
   ...
   .. math:: r_{ij} = 
	     \left[
	     \begin{array}{c} 
	       R_i^T\left(p_j - p_i\right) - \hat{p}_{ij} \\ 
	       \mathrm{Normalize}\left(\psi_j - \psi_i - \hat{\psi}_{ij}\right) 
	     \end{array}
	     \right]
   ...
   i.e. :math:`\Sigma_{ij}^{-\frac{1}{2}} r_{ij}` where :math:`\Sigma_{ij}` is
   the covariance.
   ...
   the :func:`AngleLocalParameterization::operator()` function to be:
