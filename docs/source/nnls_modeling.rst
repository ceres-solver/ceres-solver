...
parameterization. ``EigenQuaternionParameterization`` uses the same
``Plus`` operation as ``QuaternionParameterization`` but takes into
...
part of a parameter block constant by specifying the set of
...
``HomogeneousVectorParameterization`` defines a
...
``IdentityParameterization`` parameterization. So how do we
...
manifolds available, ``ProductParameterization`` can be used to
