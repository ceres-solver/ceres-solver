...
.. function:: bool CostFunction::Evaluate(double const* const* parameters, double* residuals, double** jacobians)
   ...
   If ``jacobians`` is ``NULL``, the user is only expected to compute the
   residuals.

   ``jacobians[i]`` is row-major array of size ``num_residuals x
   ...
   ``parameters[i]`` and store it in this array.
