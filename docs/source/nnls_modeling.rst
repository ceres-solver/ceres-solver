...
.. class:: GradientChecker
    ...
    cost function (by the user) multiplied by the
    `Manifold::PlusJacobian`, :math:`J'_{ij}` is the jacobian as
    computed by finite differences, multiplied by the
    `Manifold::PlusJacobian` as well, and :math:`r` is the relative
    precision.
...
In sensor fusion problems, often we have to model quantities that live
in spaces known as
`Manifolds <https://en.wikipedia.org/wiki/Manifold>`_, for example the
...
tangent to the sphere and projecting back onto the sphere will move
you away from the point you started from but moving along the normal
at the same point and the projecting back onto the sphere brings you
back to the point.
...
Defining :math:`\boxplus` and :math:`\boxminus` are defined in terms
of the matrix :math:`\exp` and :math:`\log` operations as follows:
...
part. :math:`q_1` is the coefficient of :math:`i`, :math:`q_1` is the
...
-----------------------
TODO(sameeragarwal): Update this once the port lands.
...
Create a :math:`Manifold` with Jacobians computed via automatic differentiation.
...
.. NOTE::
   ...
   ships with optimized production grade :math:`QuaternionManifold`
   implementation.
...
.. class:: Problem
   ...
   **Ownership**
   ...
   Note that even though the Problem takes ownership of these objects,
