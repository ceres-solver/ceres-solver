...
.. class:: NumericDiffCostFunction
   ...
   **WARNING** If your cost function depends on a parameter block that
   must lie on a manifold and the functor cannot be evaluated for
   values of that parameter block not on the manifold then you may
   have problems using it with Ceres.
   ...
   a cost functor depends on. This perturbation is done assuming that
   each parameter block lives in an Eucliean space. As a result some
   of the perturbations may not lie on the manifold corresponding to
   the parameter block. For example if your function depends on a
   four dimensional parameter block that is interpreted as a unit
   Quaternion. The perturbations generated by the numeric
   differentiation code will violate the unit norm property of the
   parameter block.

   To do this right, :class:`NumericDiffCostFunction` needs to be
   aware of the :class:`LocalParameterization` associated with each
...
.. class:: DynamicNumericDiffCostFunction
   ...
   **WARNING** The same caution about parameter blocks lying on
   manifolds applies here as is the case with
   :class:`NumericDiffCostFunction``.
...
numeric type as input, as long as it can be
