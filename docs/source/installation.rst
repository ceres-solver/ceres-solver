...
#. Get dependencies; unpack them as subdirectories in ``ceres/``
   ...
   #. Alternatively, Ceres Solver supports ``SuiteSparse`` binary
      ...
      .. note::
          ...
          You will need the GPL licensed version of the SuiteSparse package.
...
#. Make a dir ``ceres/ceres-bin`` (for an out-of-tree build)
   ...
   #. If you use the above binary ``SuiteSparse`` package, make sure CMake can
      ...
      Windows command prompt this can be done as follows:
...
#. Try running ``Configure``. It won't work. It'll show a bunch of options.
   ...
   #. (SuiteSparse binary package) ``BLAS_blas_LIBRARY`` and
      ...
      ``<path>/lib/blas.lib`` and ``<path>/lib/lapack.lib`` in the unzipped
      package directory.
...
.. note::
    ...
    #. The default build is ``Debug``; consider switching it to ``Release``.
