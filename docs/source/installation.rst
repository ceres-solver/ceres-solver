...
`vckpg <https://github.com/microsoft/vcpkg>`_ is a library manager for Microsoft
...
.. note::
   ...
   Ceres Solver in vcpkg is maintained by vckpkg team members and community
   contributors. Therefore, the packaged Ceres Solver may not necessarily
   correspond to the latest release. In such case, please `create an issue or
   pull request <https://github.com/Microsoft/vcpkg/>`_ on the vcpkg repository.
...
#. Use vcpkg to install and build Ceres and all its dependencies, e.g., for 64
   ...
   Or with optional components SuiteSparse using
