...
.. class:: CostFunctionToFunctor
   ...
   derivatives. Suppose, we that were not the case and
  ...
  i.e., it can just compute its value, and you would like to use
  numeric differentiation to compute its derivatives. In this case we
  can use a combination of ``NumericDiffCostFunction`` and
  ``CostFunctionToFunctor`` to get he job done.
