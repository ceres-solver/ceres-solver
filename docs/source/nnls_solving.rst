...
use of this linear solver requires that Ceres compiled with support
for at least one of:
...
Hessian, or in other words, no two parameter blocks in in the first
...
and so on. Within each group, CAMD free to order the parameter blocks
as it chooses.
