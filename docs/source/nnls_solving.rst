...
Professor Tim Davis' ``SuiteSparse`` packages [Chen]_ or the sparse
Cholesky factorization algorithm in ``Eigen`` or the sparse Cholesky
factorization routines that ship as part of Apple's ``Accelerate``
framework.
...
``Solver::Options::sparse_linear_algebra_type``.

    TODO(sameeragarwal): This needs to be updated.

1. ``sparse_linear_algebra_library_type != SUITE_SPARSE``

    The value of ``linear_solver_ordering_type`` does not matter and:

    a. ``linear_solver_type = SPARSE_NORMAL_CHOLESKY`` or
       ``linear_solver_type = CGNR`` and ``preconditioner_type =
       SUBSET``
    ...
    b. ``linear_solver_type = SPARSE_SCHUR/DENSE_SCHUR/ITERATIVE_SCHUR``
       ...
       If ``linear_solver_ordering == nullptr`` then a greedy maximum
       independent set algorithm is used to determine the first
       elimination group.

       If ``linear_solver_ordering != nullptr`` and has more than one
       elimination groups, the lowest number group is used as the
       first elimination group to compute the Schur complement. All
       other groups are ignored.

2. ``sparse_linear_algebra_library_type == SUITE_SPARSE``
    ...
    a. ``linear_solver_type = SPARSE_NORMAL_CHOLESKY`` or
       ...
       If ``linear_solver_ordering != nullptr``, then a Constrained
    ...
    b. ``linear_solver_type = SPARSE_SCHUR/DENSE_SCHUR/ITERATIVE_SCHUR``
       ...
       If ``linear_solver_ordering == nullptr`` is null then a greedy
       maximum independent set algorithm is used to compute the first
    ...
    If ``linear_solver_ordering == nullptr` then a greedy maximum
    independent set algorithm is used to compute the first elimination
    group, and Nested Dissection is applied to the corresponding Schur
    complement matrix or a subset thereof used that is used as a
    preconditioner.

    If ``linear_solver_ordering != nullptr``, the parameter blocks in
    the lowest numbered group are eliminated to compute the Schur
    complement. All other groups are ignored and Nested Dissection is
    applied to the corresponding Schur complement matrix or a subset
    thereof used that is used as a preconditioner.
    ...
    blocks in in the first elimination group should co-occur in the
    same residual block. For the best performance, this elimination
    group should be as large as possible. For standard bundle
    adjustment problems, this corresponds to the first elimination
    group containing all the 3d points, and the second containing the
    all the cameras parameter blocks.
...
.. member:: LinearSolverOrderingType Solver::Options::linear_solver_ordering_type
   ...
   Default: ``AMD``
    ...
    with O(n) storage, where as a bad ordering will result in an
    completely dense factor.
