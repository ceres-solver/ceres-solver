...
Windows port is less featureful than the Linux or Mac OS X versions due to the
unavaliability of SuiteSparse and CXSparse.

Also, building is more involved since there is no automated way to install the
dependencies.

\begin{enumerate}
  \item Make a toplevel directory for deps \& build \& src somewhere: ceres/
  \item Get dependencies; unpack them as subdirectories in ceres/ (ceres/eigen, ceres/glog, etc)
  \begin{itemize}
    \item Eigen 3.1 from eigen.tuxfamily.org (needed on windows; 3.0.x will not work) No need to build; just unpack
    \item Goolge Log. Open up the VS solution and build it.
    \item Goolge Flags. Open up the VS solution and build it.
  \end{itemize}
  \item Get git installed.
  \item Checkout Ceres git into ceres/ceres-solver.git/
 ...
 to the appropriate place where you unpacked/built them.
  ...
  \item Uncheck "SCHUR\_SPECIALIZATIONS". If it's enabled, the build will be really slow, and depending on your cost function will lead to no benefit.
...
Note: currently \texttt{system\_test} is not working properly.
Note: building Ceres as a DLL is not supported; patches welcome.
