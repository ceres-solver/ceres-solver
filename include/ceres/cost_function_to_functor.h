...
class CostFunctionToFunctor {
 ...
 private:
  ...
  bool EvaluateWithJets(const JetT** inputs, JetT* output) const {
    ...
    internal::FixedArray<double> jacobians(cost_function_->num_residuals() * kNumParameters);
    ...
    for (int i = 0; i < cost_function_->num_residuals(); ++i) {
      output[i].a = residuals[i];
      output[i].v.setZero();
      for (int j = 0; j < num_parameter_blocks; ++j) {
        for (int k = 0; k < parameter_block_sizes[j]; ++k) {
          output[i].v += jacobian_blocks[j][i * parameter_block_sizes[j] + k] * inputs[j][k].v;
        }
      }
    }
